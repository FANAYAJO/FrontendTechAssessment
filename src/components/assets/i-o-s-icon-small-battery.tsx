import React from 'react'

export default function IOSIconSmallBattery() {
  return (
    <div className="h-4 w-[25px]">
      <svg
        width="100%"
        height="100%"
        preserveAspectRatio="none"
        viewBox="0 0 25 16"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g clipPath="url(#clip0_6301_130)">
          <path
            opacity="0.35"
            d="M1.5 4.66666C1.5 3.47005 2.47005 2.5 3.66667 2.5H20.3333C21.53 2.5 22.5 3.47005 22.5 4.66667V10.6667C22.5 11.8633 21.53 12.8333 20.3333 12.8333H3.66667C2.47005 12.8333 1.5 11.8633 1.5 10.6667V4.66666Z"
            stroke="#020202"
          />
          <path
            opacity="0.4"
            d="M24 5.66669V9.66669C24.8047 9.32791 25.328 8.53982 25.328 7.66669C25.328 6.79355 24.8047 6.00546 24 5.66669Z"
            fill="#020202"
          />
          <path
            d="M3 5.33333C3 4.59695 3.59695 4 4.33333 4H19.6667C20.403 4 21 4.59695 21 5.33333V10C21 10.7364 20.403 11.3333 19.6667 11.3333H4.33333C3.59695 11.3333 3 10.7364 3 10V5.33333Z"
            fill="#020202"
          />
        </g>
        <defs>
          <clipPath id="clip0_6301_130">
            <rect width="25" height="16" fill="white" />
          </clipPath>
        </defs>
      </svg>
    </div>
  )
}
